...
def assign_track_color(command_et, color_map, source_counts):
    source = command_et['source']
    if 'color' in command_et:
        source_counts[source] = source_counts.get(source, 0) + 1
        return

    count = source_counts.get(source, 0)
    if count == 0:
        command_et['color'] = color_map[source]
    else:
        alternate_colors = list(mcolors.TABLEAU_COLORS.values())
        command_et['color'] = alternate_colors[(count - 1) % len(alternate_colors)]
...
def run(interactive, commands={}, color_map=color_map):
    ...
    if interactive:
        ...
        for command_et in commands.get('1',[]):
            ...
            assign_track_color(command_et, color_map, source_counts)
...
if __name__ == "__main__":
    ...
    star_name = 'HD 32297'
    lower_mass = 10**(0.1895 + 0.0030)
    upper_mass = 10**(0.1895 + 0.0032)
    lower_age = 0
    upper_age = 30e6
    ...
    commands={
                    '1': [ # the dynamical mass outputs go here
                        {   'source': 'MIST', # (close to Z=0.0)
                            'label': r'MIST [Fe/H] = +0.50',
                            'path_to_untarred_ET':'MIST_v1.2_feh_p0.50_afe_p0.0_vvcrit0.0_EEPS',
                        ...
                        {   'source': 'MIST', # (close to Z=0.0)
                            'label': r'MIST [Fe/H] = -0.50',
                            'path_to_untarred_ET':'MIST_v1.2_feh_m0.50_afe_p0.0_vvcrit0.0_EEPS',
                            'lower_bound_dynamical_mass_solar_mass': lower_mass,
                            'upper_bound_dynamical_mass_solar_mass': upper_mass,
                            'min_age_years': lower_age,
                            'max_age_years': upper_age,
                        ...
                        {   'source': 'MIST', # (close to Z=0.0)
                            'label': r'MIST [Fe/H] = -1.00',
                            'path_to_untarred_ET':'MIST_v1.2_feh_m1.00_afe_p0.0_vvcrit0.0_EEPS',
                            'lower_bound_dynamical_mass_solar_mass': lower_mass,
                            'upper_bound_dynamical_mass_solar_mass': upper_mass,
                            'min_age_years': lower_age,
                            'max_age_years': upper_age,
                        ...
                        {   'source': 'MIST', # (close to Z=0.0)
                            'label': r'MIST [Fe/H] = -1.50',
                            'path_to_untarred_ET':'MIST_v1.2_feh_m1.50_afe_p0.0_vvcrit0.0_EEPS',
                            'lower_bound_dynamical_mass_solar_mass': lower_mass,
                            'upper_bound_dynamical_mass_solar_mass': upper_mass,
                            'min_age_years': lower_age,
                            'max_age_years': upper_age,
                    ...
                    #             'name':'Cataldi et al. (2023a)'
                    #         },
                    #         {
                    #             'temperature_kelvin':8383,
                    #             'temperature_kelvin_err':99,
                    #             'luminosity_solar_lum':7.6,
                    #             'luminosity_solar_lum_err':0.4,
                    #             'name':'Zhang et al. (2023a)'
                    #         },
                    #    ],
                    },
