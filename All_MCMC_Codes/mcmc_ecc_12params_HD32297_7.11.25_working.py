...
def calBase():
    datafile = "HD32297Jun21_final_V2.uvfits"
    imfile = "HD32297Jun21_final_V2.fits"
    vsys = 6.0 #used to be 5.55 but we're setting it to match the circular version
...
def lnprob(p0):
    ...
    #new parameters
    logmass_stell, incl, Rc, pp, T_atm, pa, logmass, xoff, yoff, ecc, aop, vsys = p0 
    ...
    x = Disk(q = qq, McoG = Mdisk, pp=pp, Ain=R_in, Aout=R_out, Rc=Rc, incl=incl, Mstar=M_stell, Xco=X_co, vturb=v_turb, Zq0=0, Tmid0=T_mid, Tatm0=T_atm, handed=-1, ecc=ecc, aop=aop, sigbound=[1e-3,float('inf')], Rabund=[1,1000], zmax=z_max)
    ...
    total_model(x, nchans = val['nchans'] * 2 + 1, chanstep=val['chanstep'] / 2, imres=val['cell'], distance=val['distance'],freq0=val['freq'], vsys = vsys, obsv = val['obsv'], chanmin = val['chanmin'], xnpix=val['imsize'], PA=pa, offs=[xoff, yoff], modfile=model_name, flipme=flipme, hanning=hanning, Jnum = val['Jnum'],manual_chan_params=True)
    c = []
    ...
    c.append(chiSq('HD32297Jun21_final_V2.uvfits', model_name, dxy=val['cell'], dRA=0.0, dDec=0.0, pbcor=pbcor)) 
    c.append(chiSq('HD32297Jan1_final_V2.uvfits', model_name, dxy=val['cell'], dRA=0.0, dDec=0.0, pbcor=pbcor))
...
def MCMC(nsteps=4000, ndim=12, nwalkers=50, param_1=0.37, param_2=80.54, param_3=105.02, param_4= -0.37, param_5=21.3, param_6=46.6, param_7=-2.70, param_8=0.09, param_9=0.38, param_10 = 0.3, param_11 = 180.0, param_12 = 5.55, sigma_1=0.1, sigma_2=3, sigma_3=10, sigma_4=0.1, sigma_5=5, sigma_6=2, sigma_7=0.1, sigma_8=0.1, sigma_9=0.1, sigma_10 = 0.1, sigma_11 = 60, sigma_12 = 0.1, restart=False):
...
#param
    ...
    for i, result in enumerate(run):
        ...
        df.columns = ["logmass_stell", "incl", "Rc", "pp", "T_atm", "pa", "logmass", "xoff", "yoff", "ecc", "aop", "vsys", "lnprobs"]
        df.to_csv('HD32297_Jul11_2025_ecc.csv', mode='w')
